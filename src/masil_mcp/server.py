...
만드세요. 최신 상품 계약과 덱 사실을 먼저 사용하고, 문헌은 allowed_claim과 caveat 범위에서만
사용하세요. 과거 Master Q&A와 변경 이력은 질문 해석과 충돌 탐지 재료일 뿐 현재 규칙을 덮어쓰지 않습니다.

prepare_answer_context의 evidence_captures가 비어 있지 않으면 사용자가 캡처를 요청할 때까지 기다리지
말고 같은 질문으로 show_answer_evidence를 바로 호출하세요. 캡처 카드는 관련 주장의 바로 다음에 두고,
각 자료가 무엇을 뒷받침하는지만 한 줄로 말하세요. evidence_captures가 비어 있으면 억지로 이미지를
찾지 마세요. 특정 문헌 원문이나 캡처를 요청한 경우 show_evidence_capture를 사용하고, 캡처 ID는
추측하지 마세요. compare_claims만으로 캡처를 선택하지 마세요.
...
<body><h1>MASIL Knowledge Studio</h1><p>Remote MCP knowledge server</p></body>
    ...
    def show_answer_evidence(
        ...
        """Render exact captures already connected to an ordinary MASIL answer.

        Call this automatically with the same question whenever
        prepare_answer_context returns evidence_captures. Do not wait for the
        user to ask for a screenshot, and do not call it when that list is empty.
        """
