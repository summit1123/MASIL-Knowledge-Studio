...
QUERY_ALIAS_GROUPS: tuple[tuple[tuple[str, ...], str], ...] = (
    ...
    (("생활권 밖", "생활권밖", "out-of-zone", "outer zone", "unfamiliar road", "낯선 도로", "초행 도로", "처음 가는 도로", "처음 가는 경로"),
     "생활권 밖 생활권밖 out-of-zone outer zone unfamiliar road 낯선 도로 초행 도로 처음 가는 경로"),
    ...
    (("우대", "favorable"), "우대 Favorable 월 결과 안전운전 환급 후보"),
    (("기본", "standard"), "기본 Standard"),
    (("케어", "care review", "care"), "케어 Care review 월 결과 예방 안내 직원 검토 환급 차감 아님"),
    (("care가 할인", "care 할인", "케어가 할인", "케어 할인"),
     "Care review 직접 가격 영향 없음 연간 환급 별도 계산"),
    ...
    (("환급", "환급률", "refund", "premium refund", "할인율", "할인률", "discount rate", "premium discount"),
     "환급 환급률 refund rate premium refund annual refund 연간 환급"),
    ...
    (("사회적 가치", "이동권", "mobility rights"),
     "사회적 가치 이동권 mobility rights 자기조절 self-regulation Senior Society"),
    (("Jackie", "Jackie Chan", "Tom Hanks", "재키", "톰 행크스"),
     "Jackie Chan Tom Hanks dashboard same distance different behavior annual refund"),
)
